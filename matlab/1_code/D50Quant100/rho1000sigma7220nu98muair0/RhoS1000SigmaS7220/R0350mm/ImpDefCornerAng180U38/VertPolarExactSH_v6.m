...
%     end
    ...
    while abs(errorP)>=tolP && reduc == 0 
        ...
        if strcmp(warnId, 'MATLAB:nearlySingularMatrix') == true || dt*T_unit < 1e-10
            ...
            if going_back > 50
                exit = true;
                warning('Went back too many times. Stopping execution');
...
%             end
            ...
            if PROBLEM_CONSTANTS.DEBUG_FLAG == true
                fprintf("Inside ll: %0.2g, reduc: %d, errP: %0.5g \n", ll, errorP);
...
if indexes_to_save(2) == 1
    etaMatPer = etaMatPer(:, 2:end);
    phiMatPer = phiMatPer(:, 2:end);
    psMatPer  = psMatPer(:, 2:end);
    indexes_to_save = indexes_to_save(2:end); 
end
...
tvecOri = tvecOri(1:(current_to_save-1)); tvec = tvec(indexes_to_save); save('tvec.mat','tvec'); 
