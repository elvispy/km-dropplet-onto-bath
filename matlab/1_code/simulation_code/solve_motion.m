...
tstart = tic;
%data in cgs
prefix_env = getenv('SOLVE_MOTION_OUTPUT_PREFIX');
if isempty(prefix_env)
    prefix_env = '';
end
...
max_steps_env = str2double(getenv('SOLVE_MOTION_MAX_STEPS'));
use_max_steps = ~isnan(max_steps_env) && max_steps_env > 0;
if use_max_steps
    steps = max_steps_env;
    tend = t + steps * dtb;
end
...
if use_max_steps
    tvec = t:(dtb):(t+steps*dtb);
else
    tvec = t:(dtb):(tend+1);
end
tvecOri = tvec;%vector of times assuming no refinement has happened
...
try
    ...
    save(sprintf("%serror_logU0=%g-%s.mat", prefix_env, U0, a),'ME');
end % end while catch

simul_time = toc(tstart);
%simul_time = simul_time - tstart;

save(sprintf('%sProblemConditions.mat', prefix_env), "T", "N", "U0", "Ang", "Re", "Fr", "We", ...
