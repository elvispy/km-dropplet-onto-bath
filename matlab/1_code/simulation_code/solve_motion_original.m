...
function solve_motion_original(U0, ~, N, tolP, wd, debug_flag, tend_override)
...
%Physical parameter
tend = tend_override; %Earliest possible end of simulation in characteristic units
...
function results_saver(prefix, indexes, variables, variableNames)
    for ii = 1:length(variables)
       ...
       % The error is: argument to -STRUCT must be the name of a scalar structure variable.
       % save(..., '-struct', 'stru') looks for a variable named 'stru' in the workspace.
       % It seems okay, but maybe 'stru' is not scalar? It should be.
       % Or maybe I should just use save(..., 'field') style if I can't get -struct to work with dynamic fields easily in this scope context weirdness.
       % Actually, straightforward:
       s = struct();
       s.(variableNames{ii}) = var;
       save(sprintf('%s%s.mat', prefix, variableNames{ii}), '-struct', 's');
    end
