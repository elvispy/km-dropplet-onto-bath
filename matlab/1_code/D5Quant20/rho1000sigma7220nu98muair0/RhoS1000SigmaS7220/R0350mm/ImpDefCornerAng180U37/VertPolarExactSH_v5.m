...
U0 = 37; %impact velocity in cm/s (unit of velocity for the problem)
...
PROBLEM_CONSTANTS = struct("froude_nb", Fr, "weber_nb", We, ...
    ...
    "spatial_tol", dr, ...
    "DEBUG_FLAG", false, "linear_on_theta", true, ...
...
%     end
    ...
    while abs(errorP)>=tolP && reduc == 0 
        ...
        if strcmp(warnId, 'MATLAB:nearlySingularMatrix') == true || dt*T_unit < 1e-10
            ...
            tvec = tvec(tvec < tvec(tentative_index) & tvec > tvec(tentative_index) + dtb);
            g = @(a, b) a + 0.4 * (b-a);
            tvec = [tvec(1:tentative_index),...
                    g(tvec(tentative_index),tvec(tentative_index+1)),...
