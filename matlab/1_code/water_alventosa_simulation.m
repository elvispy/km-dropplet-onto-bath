% This will will simulate the special case from
% Alventosa et al 2023 specifically, which is a water-water
% drop-bath impact dynamics at an impact velocity of about U = 39 cm/s
% Define parameters (scalars or vectors allowed), with unit notes
D = 50;                    % Unitless
Quant = 50;               % Unitless

% aux functions
weFunction = @(rho, v, L, sigma) (rho .* v.^2 .* L) ./ sigma;
ohFunction = @(mu, rho, sigma, L) mu ./ sqrt(rho .* sigma .* L);
boFunction = @(rho, g, L, sigma) (rho .* g .* L.^2) ./ sigma;


rho = 1;                % (g/cm^3) ? multiply by 1000 for kg/m^3
sigma = 72.20;             % (dyn/cm) ? multiply by 100 for N/m
nu = 0.98E-2;                 % (St = cm^2/s) ? multiply by 1e-4 for m^2/s
muair = 0;                 % (g/cm·s) if applicable
RhoS = 1;               % (g/cm^3) ? multiply by 1000 for kg/m^3
SigmaS = 72.20;            % (dyn/cm) ? multiply by 100 for N/m
R = 0.035;% (cm) ? multiply by 0.01 for m
Ang = 180;                 % (deg)
%v = @(we, rho, l, sigma) sqrt(we .* sigma ./ (l .* rho));
%We = logspace(-1, 0, 11); We = We(We > 0.26); We = We(1:(end-1));
U = 38;   % (cm/s) ? multiply by 0.01 for m/s
