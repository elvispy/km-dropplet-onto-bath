...
for ii = 1:length(files)
    ...
    if is_valid(files.folder(ii), data)
        
        if length(files) == 1; folder_name = files.folder; else folder_name = files.folder(ii); end
        cd(folder_name);
        load("U0.mat");
        cd ..
        ...
        max_deflection = abs(max_def); 
        coef_restitution = CRref;
        contact_time = tcont;

        plotting_data = [plotting_data; {We, Bo, Oh, max_deflection, coef_restitution, contact_time}];
   
    end
end

% Maximum deflection pot
Max_Def_Plot = scatter(plotting_data.We,plotting_data.max_deflection,'color',[ 0.4660    0.6740    0.1880],'LineWidth',4);
